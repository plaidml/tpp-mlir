...
extern void simple_copy(DECL_VEC2D_FUNC_IN_ARGS(input, float), DECL_VEC2D_FUNC_OUT_ARGS(output, float));

/* Initialize matrix with value x+y at position (x, y) */
void init_matrix(struct vec_f2d* m) {
  for (int64_t y = 0; y < m->sizes[0]; y++)
    for (int64_t x = 0; x < m->sizes[1]; x++) vec_f2d_set(m, x, y, x + y);
}
...
int main(int argc, char** argv) {
  ...
  if (vec_f2d_alloc(&input, n, m) || vec_f2d_alloc(&output, n, m) || vec_f2d_alloc(&output_ref, n, m)) {
