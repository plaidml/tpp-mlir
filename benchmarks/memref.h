 ...
 * 2d memref */
#define VEC2D_ARGS(v) \
  (v)->allocatedPtr, (v)->alignedPtr, (v)->offset, (v)->sizes[0], (v)->sizes[1], (v)->strides[0], (v)->strides[1]

/* Forwards all arguments declared with DECL_VEC2D_FUNC_ARGS to
 * another function */
#define VEC2D_FWD_ARGS(prefix) \
  prefix##_allocatedPtr, prefix##_alignedPtr, prefix##_offset, prefix##_size0, prefix##_size1, prefix##_stride0, prefix##_stride1

#define DECL_VEC2D_FUNC_ARGS(prefix, eltype, cst) \
  cst eltype *prefix##_allocatedPtr, cst eltype *prefix##_alignedPtr, int64_t prefix##_offset, int64_t prefix##_size0, \
    int64_t prefix##_size1, int64_t prefix##_stride0, int64_t prefix##_stride1

#define DECL_VEC2D_FUNC_IN_ARGS(prefix, eltype) DECL_VEC2D_FUNC_ARGS(prefix, eltype, const)

#define DECL_VEC2D_FUNC_OUT_ARGS(prefix, eltype) DECL_VEC2D_FUNC_ARGS(prefix, eltype, )
...
static inline void memref_2d_dump_metainfo(DECL_VEC2D_FUNC_ARGS(m, void, const)) {
  printf("2d memref:\n"
         ...
         "  stride0: %" PRIu64 "\n"
         "  stride1: %" PRIu64 "\n",
    m_allocatedPtr, m_alignedPtr, m_offset, m_size0, m_size1, m_stride0, m_stride1);
}
...
#define DECL_VEC2D_FUNCTIONS(name, eltype, format) \
  ...
  static inline eltype name##_get(const struct name* v, int64_t x, int64_t y) { return *(v->alignedPtr + y * v->sizes[1] + x); } \
  /* Assigns `f` to the element at position (`x`, `y`) of a 2d                 \
   * memref `v`                                                                \
   */ \
  static inline void name##_set(struct name* v, int64_t x, int64_t y, eltype f) { *(v->alignedPtr + y * v->sizes[1] + x) = f; } \
  /* Compares the values of two 2d memrefs. Returns 1 if they are              \
   * equal, otherwise 0.                                                       \
   */ \
  static inline int name##_compare(const struct name* a, const struct name* b) { \
    /* Compare shapes */ \
  ...
  static inline void name##_dump(const struct name* v) { \
    for (int64_t y = 0; y < v->sizes[0]; y++) { \
      for (int64_t x = 0; x < v->sizes[1]; x++) { \
        printf(format "%s", *(v->alignedPtr + y * v->sizes[1] + x), x == v->sizes[1] - 1 ? "" : " "); \
      } \
      puts(""); \
    } \
  }
