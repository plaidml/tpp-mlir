...
struct BenchOptions {
  llvm::cl::opt<std::string> inputDims{"input",
      llvm::cl::desc("Input dimensions, 1/2/3/4D"),
      llvm::cl::value_desc("MxNxKxB"), llvm::cl::init("")};
  llvm::cl::opt<bool> xsmm{"xsmm",
      llvm::cl::desc("Run XSMM optimized version"),
      llvm::cl::init(false)};
  llvm::cl::opt<int> iter{"iter",
  ...
  llvm::cl::opt<bool> verbose{"v",
      ...
      llvm::cl::init(false)};
  llvm::cl::opt<bool> gflops{"gflops",
      llvm::cl::desc("Emit results in GFLOP/s (default is ms)"),
      llvm::cl::init(false)};
};
} // namespace
...
struct BenchConfig {
  ...
  BenchConfig(int argc, char**argv) {
    ...
    // Parse input dims
    size_t m[4];
    int found = sscanf(options.inputDims.c_str(), "%ldx%ldx%ldx%ld", &m[0], &m[1], &m[2], &m[3]);
    if (found > 0)
    ...
    if (!iter) {
      ...
      if (dims.size() != 0) {
        ...
        float size = std::accumulate(dims.begin(), dims.end(), 1.0, std::multiplies<float>());
