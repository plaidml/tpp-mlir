...
public:
  ...
  /// Get dimensions
  const ListArg& getDims() {
    return dims;
  }

  /// Get specific dimension
  unsigned getDim(unsigned i) const {
    return dims[i];
  }
...
template <class T>
struct EmptyTensor : public Tensor<T> {
  EmptyTensor(InitArg dims) : Tensor<T>(dims) {
    this->clear();
  }
...
template <class T>
struct ConstantTensor : public Tensor<T> {
  ConstantTensor(InitArg dims)
    : Tensor<T>(dims) {
