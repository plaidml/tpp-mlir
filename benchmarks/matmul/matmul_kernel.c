...
void matmul(DECL_VEC2D_FUNC_IN_ARGS(a, float), DECL_VEC2D_FUNC_IN_ARGS(b, float), DECL_VEC2D_FUNC_OUT_ARGS(c, float)) {
  ...
  LIBXSMM_INLINE_XGEMM(
    float, float, "N", "N", &m, &n, &k, &alpha, b_alignedPtr, &lda, a_alignedPtr, &ldb, &beta, c_alignedPtr, &ldc);
#else
  const libxsmm_gemm_shape gemm_shape = libxsmm_create_gemm_shape(
    m, n, k, lda, ldb, ldc, LIBXSMM_DATATYPE(float), LIBXSMM_DATATYPE(float), LIBXSMM_DATATYPE(float), LIBXSMM_DATATYPE(float));
  libxsmm_xmmfunction kernel = {NULL};
  kernel.gemm = libxsmm_dispatch_gemm_v2(gemm_shape, LIBXSMM_GEMM_FLAG_NONE, LIBXSMM_PREFETCH_NONE);
