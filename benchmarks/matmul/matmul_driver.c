...
extern void matmul(DECL_VEC2D_FUNC_IN_ARGS(a, float), DECL_VEC2D_FUNC_IN_ARGS(b, float), DECL_VEC2D_FUNC_OUT_ARGS(c, float));


/* Reference implementation of a matrix multiplication */
void matmul_refimpl(const struct vec_f2d* a, const struct vec_f2d* b, struct vec_f2d* c) {
  const int64_t m = c->sizes[1], n = c->sizes[0], k = a->sizes[1];
  for (int64_t ni = 0; ni < n; ++ni) {
    for (int64_t ki = 0; ki < k; ++ki) {
      for (int64_t mi = 0; mi < m; ++mi) {
        vec_f2d_set(c, mi, ni, vec_f2d_get(a, ki, ni) * vec_f2d_get(b, mi, ki) + vec_f2d_get(c, mi, ni)); // beta=1
      }
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  if (EXIT_SUCCESS == vec_f2d_alloc(&a, m, k) && EXIT_SUCCESS == vec_f2d_alloc(&b, k, n) &&
      EXIT_SUCCESS == vec_f2d_alloc(&c, m, n) && EXIT_SUCCESS == vec_f2d_alloc(&d, m, n))
  {
    init_matrix(&a);
    init_matrix(&b);
    ...
    if (0 >= nrepeat) {
      nrepeat = (0 < duration ? (int)LIBXSMM_ROUND(max_duration / duration) : nwarmup);
    }
    if (nwarmup > nrepeat) {
      nrepeat = nwarmup;
    }
    ...
    if (0 == vec_f2d_compare(&c, &d) || 3 <= verbose || 0 > verbose) {
      ...
      double error = 0.0;
      libxsmm_matdiff(&diff, LIBXSMM_DATATYPE_F32, n, m, d.alignedPtr, c.alignedPtr, &n, &n);
