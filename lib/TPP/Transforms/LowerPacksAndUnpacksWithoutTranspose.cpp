...
void lowerPackWithoutTranspose(tensor::PackOp packOp,
...
struct LowerPackOnInputsWithoutTranspose
  ...
  LowerPackOnInputsWithoutTranspose(MLIRContext *context,
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
    ...
    for (auto operandIdx : {0, 1}) {
      ...
      lowerPackWithoutTranspose(packOp, genericOp, operandIdx, rewriter);
...
struct LowerPackUnpackOnOutputWithoutTranspose
    : public OpRewritePattern<linalg::GenericOp> {
  // Is only called with single-user packOp operands, so callback can always
  // find the (use by the) linalg.generic that is the target of the pattern.
  ...
  LowerPackUnpackOnOutputWithoutTranspose(MLIRContext *context,
                                          ControlFn controlFn = nullptr,
                                          PatternBenefit benefit = 1)
      : OpRewritePattern(context, benefit), controlFn(std::move(controlFn)) {}
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
    ...
    lowerPackWithoutTranspose(packOp, genericOp, /*operandIdx=*/2, rewriter);
    auto res = linalg::lowerUnPack(rewriter, unpackOp);

    // Set genericOp's result type to the adjusted type of the out parameter.
    genericOp->getOpResult(0).setType(genericOp.getOperand(2).getType());
    ...
    // we know the underlying buffer will be used as is and hence we do not need
    // to specify a dest to update into.
...
struct LowerPacksAndUnpacksWithoutTranspose
  ...
  void runOnOperation() override {
    ...
    patterns.add<LowerPackOnInputsWithoutTranspose>(
    ...
    patterns.add<LowerPackUnpackOnOutputWithoutTranspose>(ctx);
