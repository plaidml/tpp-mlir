...
func.func @xsmm_dialect(%arg0: memref<2x2xf32>,
                        %arg1: memref<2x2xf32>, %arg2: memref<2x2xf32>, %arg3: memref<3x2x2xf32>) {

  %d = arith.constant 0 : i64
  // CHECK: xsmm.binary add
  xsmm.binary add(data_type = f32, %d, %arg0, %arg1, %arg1)
    : (i64, memref<2x2xf32>, memref<2x2xf32>, memref<2x2xf32>) -> ()

  // CHECK: xsmm.binary sub
  xsmm.binary sub(data_type = f32, %d, %arg0, %arg1, %arg1)
    : (i64, memref<2x2xf32>, memref<2x2xf32>, memref<2x2xf32>) -> ()

  // CHECK: xsmm.binary div
  xsmm.binary div(data_type = f32, %d, %arg0, %arg1, %arg1)
    : (i64, memref<2x2xf32>, memref<2x2xf32>, memref<2x2xf32>) -> ()

  // CHECK: xsmm.unary relu
  xsmm.unary relu(data_type = f32, %d, %arg0, %arg0)
  ...
  %2 = xsmm.gemm.dispatch [1, 2, 3, 4, 5, 5] flags = (none) data_type = f32
  // CHECK-NEXT: xsmm.gemm.dispatch
  %3 = xsmm.gemm.dispatch [1, 2, 3, 4, 5, 6] flags = (beta_0) data_type = f32
  // CHECK-NEXT: xsmm.gemm.dispatch
  %4 = xsmm.gemm.dispatch [1, 2, 3, 4, 5, 6] flags = (beta_0) data_type = bf16
  // CHECK-NEXT: xsmm.gemm.dispatch
  %5 = xsmm.gemm.dispatch [1, 2, 3, 4, 5, 6] flags = (vnni_a, vnni_b) data_type = bf16
  // CHECK-NEXT: xsmm.brgemm.dispatch
  %6 = xsmm.brgemm.dispatch [1, 2, 3, 4, 5, 6, 1, 1] flags = (vnni_a, vnni_b) data_type = bf16
  // CHECK-NEXT: xsmm.brgemm.dispatch
  %7 = xsmm.brgemm.dispatch [1, 2, 3, 4, 5, 6, 1, 1] flags = (beta_0) data_type = bf16
  // CHECK-NEXT: xsmm.brgemm.dispatch
  %8 = xsmm.brgemm.dispatch [1, 2, 3, 4, 5, 6, 1, 1] flags = (beta_0) data_type = f32
  // CHECK-NEXT: xsmm.brgemm.dispatch
  %9 = xsmm.brgemm.dispatch [1, 2, 3, 4, 5, 6, 1, 1] flags = (none) data_type = f32
  // CHECK: xsmm.gemm.dispatch {{.*}} {myAttr = "myattr"}
  %10 = xsmm.gemm.dispatch [1, 2, 3, 4, 5, 6] flags = (none) data_type = f32 {myAttr = "myattr"}
  ...
  %12 = xsmm.fused_brgemm.dispatch [1, 2, 3, 4, 5, 6, 1, 1] [add, relu]
