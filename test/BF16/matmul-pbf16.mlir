// RUN: tpp-opt %s -convert-linalg-to-loops -convert-tpp-to-xsmm -convert-xsmm-to-func  -convert-vector-to-scf -convert-scf-to-cf -lower-affine -convert-vector-to-llvm -convert-memref-to-llvm -arith-expand -convert-math-to-llvm -convert-func-to-llvm -reconcile-unrealized-casts |\
// RUN: mlir-cpu-runner \
...
func.func @entry() {
  ...
  %db = memref.alloc() : memref<4x4x2xbf16>
  linalg.fill ins(%f0 : bf16) outs (%db : memref<4x4x2xbf16>)
  
  // Call kernel.
  %dc = memref.alloc() : memref<4x4xbf16>
  %zero = arith.constant 0.0 : bf16
  linalg.fill ins(%zero : bf16) outs(%dc : memref<4x4xbf16>)
  
  call @matmultpp(%da, %db, %dc)
  ...
  %v0 = vector.transfer_read %dc[%c0, %c0], %d1 : memref<4x4xbf16>, vector<4x4xbf16>
