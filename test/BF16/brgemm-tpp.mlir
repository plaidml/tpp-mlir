// RUN: tpp-opt -transform-dialect-interpreter -empty-tensor-to-alloc-tensor -one-shot-bufferize="bufferize-function-boundaries allow-return-allocs function-boundary-type-conversion=identity-layout-map" --convert-vnni-to-tpp %s | FileCheck %s
...
// CHECK: %[[PACK:.+]] = memref.alloc() {alignment = 64 : i64} : memref<32x2x4x2xbf16>
