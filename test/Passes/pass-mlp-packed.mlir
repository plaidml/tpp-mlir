// RUN: tpp-opt %s -pack-matmul="block-factors=32,32,32" -propagate-pack-and-unpack -canonicalize -tile-consumer-and-fuse-producers="tile-sizes=1,1" -canonicalize -generalize-tensor-pack-unpack -empty-tensor-to-alloc-tensor -one-shot-bufferize="bufferize-function-boundaries allow-return-allocs function-boundary-type-conversion=identity-layout-map" -canonicalize -drop-equivalent-buffer-results -finalizing-bufferize -canonicalize -convert-linalg-to-tpp="use-parallel-loops=false" -rewrite-to-brgemm -convert-linalg-to-tpp | FileCheck %s
...
func.func @main(%arg0: tensor<128x256xf32>, %arg1: tensor<256x512xf32>,
  ...
  // CHECK: tpp.brgemm ins(%{{.+}} : memref<8x32x32xf32, strided<[1024, 32, 1], offset: ?>>, %{{.+}} : memref<8x32x32xf32, strided<[1024, 32, 1], offset: ?>>) out(%{{.*}} : memref<32x32xf32, strided<[32, 1], offset: ?>>)
  %2 = linalg.matmul ins(%arg0, %arg1 : tensor<128x256xf32>, tensor<256x512xf32>) outs(%1 : tensor<128x512xf32>) -> tensor<128x512xf32>
  // CHECK-NEXT: tpp.relu ins(%{{.+}} : memref<32x32xf32, strided<[32, 1], offset: ?>>) out(%{{.+}} : memref<32x32xf32, strided<[32, 1], offset: ?>>)
  ...
  // CHECK: tpp.brgemm ins(%{{.+}} : memref<16x32x32xf32, strided<[1024, 32, 1], offset: ?>>, %{{.+}} : memref<16x32x32xf32, strided<[1024, 32, 1], offset: ?>>) out(%{{.+}} : memref<32x32xf32, strided<[32, 1], offset: ?>>)
  ...
  // CHECK: tpp.brgemm ins(%{{.+}} : memref<32x32x32xf32, strided<[1024, 32, 1], offset: ?>>, %{{.+}} : memref<32x32x32xf32, strided<[1024, 32, 1], offset: ?>>) out(%{{.+}} : memref<32x32xf32, strided<[32, 1], offset: ?>>)
  ...
  // CHECK: tpp.brgemm ins(%{{.+}} : memref<64x32x32xf32, strided<[1024, 32, 1], offset: ?>>, %{{.+}} : memref<64x32x32xf32, strided<[1024, 32, 1], offset: ?>>) out(%{{.+}} : memref<32x32xf32, strided<[32, 1], offset: ?>>)
