...
// CHECK:       %[[vec1:.*]] = vector.transfer_read %[[expand_shape]][%c0, %c0, %c0, %c0], %cst {in_bounds = [true, true, true, true, true], permutation_map = #map} : memref<4x4x2x2xbf16, strided<[16, 4, 2, 1], offset: ?>>, vector<4x2x4x4x2xbf16>
// CHECK:       %[[vec2:.*]] = vector.transfer_read %0[%c0, %c0, %c0, %c0], %cst {in_bounds = [true, true, true, true, true], permutation_map = #map1} : memref<4x2x4x2xbf16>, vector<4x2x4x4x2xbf16>
// CHECK:       %[[vec3:.*]] = vector.transfer_read %[[subview]][%c0, %c0], %cst {in_bounds = [true, true]} : memref<4x4xbf16, strided<[4, 1], offset: ?>>, vector<4x4xbf16>
// CHECK:       %[[vec4:.*]] = arith.mulf %[[vec1]], %[[vec2]] : vector<4x2x4x4x2xbf16>
// CHECK:       %[[vec5:.*]] = vector.multi_reduction <add>, %[[vec4]], %[[vec3]] [0, 1, 4] : vector<4x2x4x4x2xbf16> to vector<4x4xbf16>
// CHECK:       vector.transfer_write %[[vec5]], %[[subview]][%c0, %c0] {in_bounds = [true, true]} : vector<4x4xbf16>, memref<4x4xbf16, strided<[4, 1], offset: ?>>
