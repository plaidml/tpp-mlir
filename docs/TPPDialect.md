...
The TPP dialect follows other MLIR dialects and uses the _destination passing_ style (using `ins` and `outs` for inputs and outputs).

At `tensor` level:
```mlir
  // Empyt as out-of-place
  %C = tensor.empty : tensor <>
  %A = tpp.add ins(%arg0, %arg1) outs(%C) : (tensor<>, tensor<>) -> tensor<>
  // Reusing the same tensor as in-place
  %M = tpp.gemm ins(%A, %B, %A) outs(%C) : (tensor<>, tensor<>, tensor<>) -> tensor<>
```
...
```mlir
  ...
  tpp.gemm ins(%A, %B, %buf) outs(%buf) : (memref<>, memref<>, memref<>)
```

### Unary

Unary operations have one `ins` argument and one `outs` argument.
...
Binary operations have both `ins` and `outs`, with always two `ins` and one `outs` arguments.
...
Ternary operations have both `ins` and `outs`, with always three `ins` and one `outs` arguments.
...
Most operations are assumed to be out-of-place.
